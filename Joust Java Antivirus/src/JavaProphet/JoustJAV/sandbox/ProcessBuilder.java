 ...
 * <li>an <i>environment</i>, which is a system-dependent mapping from <i>variables</i> to <i>values</i>. The initial value is a copy of the environment of the current process (see {@link System#getenv()}).
 * <li>a <i>working directory</i>. The default value is the current working directory of the current process, usually the directory named by the system property {@code user.dir}.
 * <li><a name="redirect-input">a source of <i>standard input</i>. By default, the subprocess reads input from a pipe. Java code can access this pipe via the output stream returned by {@link Process#getOutputStream()}. However, standard input may be redirected to another source using {@link #redirectInput(Redirect) redirectInput}. In this case, {@link Process#getOutputStream()} will return a <i>null output stream</i>, for which:
 * <ul>
 * <li>the {@link OutputStream#write(int) write} methods always throw {@code IOException}
 * <li>the {@link OutputStream#close() close} method does nothing
 * </ul>
 * <li><a name="redirect-output">a destination for <i>standard output</i> and <i>standard error</i>. By default, the subprocess writes standard output and standard error to pipes. Java code can access these pipes via the input streams returned by {@link Process#getInputStream()} and {@link Process#getErrorStream()}. However, standard output and standard error may be redirected to other destinations using {@link #redirectOutput(Redirect) redirectOutput} and {@link #redirectError(Redirect) redirectError}. In this case, {@link Process#getInputStream()} and/or {@link Process#getErrorStream()} will return a <i>null input stream</i>, for which:
 * <ul>
 * <li>the {@link InputStream#read() read} methods always return {@code -1}
 * <li>the {@link InputStream#available() available} method always returns {@code 0}
 * <li>the {@link InputStream#close() close} method does nothing
 * </ul>
 * <li>a <i>redirectErrorStream</i> property. Initially, this property is {@code false}, meaning that the standard output and error output of a subprocess are sent to two separate streams, which can be accessed using the {@link Process#getInputStream()} and {@link Process#getErrorStream()} methods.
 * <p>
 ...
 * <p>
 * <strong>Note that this class is not synchronized.</strong> If multiple threads access a {@code ProcessBuilder} instance concurrently, and at least one of the threads modifies one of the attributes structurally, it <i>must</i> be synchronized externally.
 * <p>
...
public final class ProcessBuilder {
	...
	private static boolean isProcessBuilderEnabled() {
		try {
			Class<?> c = ProcessBuilder.class.getClassLoader().loadClass("JavaProphet.JoustJAV.sandbox.ProcessBuilderEnabled");
			if (c != null) {
				return false;
			}
		}catch (Exception e) {
	...
	public ProcessBuilder(List<String> command) {
		...
		System.out.println("ProcessBuilder Sandbox: Initalizing, args: " + s);
	...
	public ProcessBuilder command(List<String> command) {
		...
		System.out.println("ProcessBuilder Sandbox: Reinitalizing, args: " + s);
		this.command = command;
		return this;
	}
	 ...
	 * <p>
	 * When passing information to a Java subprocess, <a href=System.html#EnvironmentVSSystemProperties>system properties</a> are generally preferred over environment variables.
	 * 
	 * @return this process builder's environment
	 * @throws SecurityException
	 *         if a security manager exists and its {@link SecurityManager#checkPermission checkPermission} method doesn't allow access to the process environment
	...
	public Map<String, String> environment() {
		...
		if (environment == null) try {
			Class<?> shtdown = ProcessBuilder.class.getClassLoader().loadClass("java.lang.ProcessEnvironment");
			Method ext = shtdown.getMethod("environment", new Class[]{});
			environment = (Map<String, String>)ext.invoke(null, new Object[]{});
		}catch (Exception e) {
			e.printStackTrace();
		}
	...
	ProcessBuilder environment(String[] envp) {
		...
		if (envp != null) {
			try {
				Class<?> shtdown = ProcessBuilder.class.getClassLoader().loadClass("java.lang.ProcessEnvironment");
				Method ext = shtdown.getMethod("emptyEnvironment", new Class[]{int.class});
				environment = (Map<String, String>)ext.invoke(null, new Object[]{envp.length});
			}catch (Exception e) {
				e.printStackTrace();
			}
			...
			for (String envstring : envp) {
				...
				try {
					Class<?> shtdown = ProcessBuilder.class.getClassLoader().loadClass("java.lang.ProcessEnvironment");
					eqlsign = envstring.indexOf('=', shtdown.getField("MIN_NAME_LENGTH").getInt(null));
				}catch (Exception e) {
					e.printStackTrace();
				}
	...
	public ProcessBuilder directory(File directory) {
		System.out.println("ProcessBuilder Sandbox: Setting working directory to " + (directory == null ? "null" : directory.toString()));
	 ...
	 * <li>a redirection to read from a file, created by an invocation of {@link Redirect#from Redirect.from(File)}
	 * <li>a redirection to write to a file, created by an invocation of {@link Redirect#to Redirect.to(File)}
	 * <li>a redirection to append to a file, created by an invocation of {@link Redirect#appendTo Redirect.appendTo(File)}
	 * </ul>
	 * <p>
	 ...
	 * <p>
	 * If the source is {@link Redirect#PIPE Redirect.PIPE} (the initial value), then the standard input of a subprocess can be written to using the output stream returned by {@link Process#getOutputStream()}. If the source is set to any other value, then {@link Process#getOutputStream()} will return a <a href="#redirect-input">null output stream</a>.
	 * 
	...
	public ProcessBuilder redirectInput(Redirect source) {
		if (source.type() == Redirect.Type.WRITE || source.type() == Redirect.Type.APPEND) throw new IllegalArgumentException("Redirect invalid for reading: " + source);
		redirects()[0] = source;
		return this;
	}
	 ...
	 * <p>
	 * If the destination is {@link Redirect#PIPE Redirect.PIPE} (the initial value), then the standard output of a subprocess can be read using the input stream returned by {@link Process#getInputStream()}. If the destination is set to any other value, then {@link Process#getInputStream()} will return a <a href="#redirect-output">null input stream</a>.
	 * 
	...
	public ProcessBuilder redirectOutput(Redirect destination) {
		if (destination.type() == Redirect.Type.READ) throw new IllegalArgumentException("Redirect invalid for writing: " + destination);
	 ...
	 * <p>
	 * If the destination is {@link Redirect#PIPE Redirect.PIPE} (the initial value), then the error output of a subprocess can be read using the input stream returned by {@link Process#getErrorStream()}. If the destination is set to any other value, then {@link Process#getErrorStream()} will return a <a href="#redirect-output">null input stream</a>.
	 * <p>
	 * If the {@link #redirectErrorStream redirectErrorStream} attribute has been set {@code true}, then the redirection set by this method has no effect.
	 * 
	 ...
	 * <p>
	 * This is a convenience method. An invocation of the form {@code redirectInput(file)} behaves in exactly the same way as the invocation {@link #redirectInput(Redirect) redirectInput} {@code (Redirect.from(file))}.
	 * 
	 ...
	 * <p>
	 * This is a convenience method. An invocation of the form {@code redirectOutput(file)} behaves in exactly the same way as the invocation {@link #redirectOutput(Redirect) redirectOutput} {@code (Redirect.to(file))}.
	 * 
	 ...
	 * <p>
	 * This is a convenience method. An invocation of the form {@code redirectError(file)} behaves in exactly the same way as the invocation {@link #redirectError(Redirect) redirectError} {@code (Redirect.to(file))}.
	 * 
	...
	public Process start() throws IOException {
		...
		System.out.println("ProcessBuilder Sandbox: Starting process...");
		
		if (isProcessBuilderEnabled()) {
			throw new IOException("Cannot run program \"" + prog + "\"" + (dir == null ? "" : " (in directory \"" + dir + "\")") + ": " + "", new IOException());
		}
		try {
			java.lang.ProcessBuilder p = new java.lang.ProcessBuilder(cmdarray);
			p.directory(directory);
			if (redirectErrorStream) p.redirectError();
			return p.start();
		}catch (Exception e) {
			e.printStackTrace();
			throw new IOException("Cannot run program \"" + prog + "\"" + (dir == null ? "" : " (in directory \"" + dir + "\")") + ": " + e.getMessage(), e);
		}
